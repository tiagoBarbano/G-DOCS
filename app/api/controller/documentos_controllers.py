...
from app.api.service.documento_service import (
    start_update_one_documento,
    inativar_documento,
    expurgar_documentos_por_data,
)
from typing import Annotated
from sqlalchemy.ext.asyncio import AsyncSession
from app.api.database.repository.documentos_repository import (
    add_documento,
    get_all_documentos,
...
class DocumentosController:
    ...
    async def get_documento_doc_id(
        self, doc_id: uuid.UUID, db: AsyncSession = Depends(get_db)
    ) -> DocumentosShema:
    ...
    async def upload_one_documento(
        ...
        try:
            ...
            documento_upload = await start_update_one_documento(
                path_documento, area, db, arq, my_uuid, s3
            )

            ret = {
                "message": "Arquivo Salvo no S3",
                "id_doc": str(my_uuid),
    ...
    @controller.route.post(
        "/documentos_url/{path_documento}", status_code=status.HTTP_201_CREATED
    )
    async def upload_one_documento_url(
        self,
        path_documento: str,
        background_tasks: BackgroundTasks,
        area_request: Annotated[str | None, Header()],
        ...
        try:
            ...
            caminho_documento = f"{area.caminho_area}/{path_documento}/{arq.filename}"

            url = s3.generate_presigned_url(
                "get_object",
                Params={"Bucket": settings.bucket_s3, "Key": caminho_documento},
                ExpiresIn=60,
            )
            ...
            new_doc = DocumentoModel(
                ...
                my_uuid=my_uuid,
                nome_documento=arq.filename,
                caminho_documento=caminho_documento,
                status_documento=StatusTypes.pendente.name,
    ...
    async def expurgo_documento(self, db: AsyncSession = Depends(get_db)):
        try:
            ...
            if flag_expurgo is True:
                ...
                message = "Não existem documentos para expurgo"

            return ORJSONResponse(content=message, status_code=status.HTTP_200_OK)
        except HTTPException as ex:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="problema no expurgo"
            )

    @controller.route.get("/documento/download_url/{id_doc}")
    async def download_documento_url(
        self,
        id_doc: uuid.UUID,
        db: AsyncSession = Depends(get_db),
        s3: AsyncSession = Depends(client_s3),
    ...
    async def download_stream_documento(
        self,
        id_doc: uuid.UUID,
        db: AsyncSession = Depends(get_db),
        s3=Depends(client_s3),
        ...
        result = s3.get_object(
            Bucket=settings.bucket_s3, Key=documento.caminho_documento
        )
        ...
        return StreamingResponse(
            file_like, headers=headers, media_type="application/octet-stream"
        )
